...
public class Server5 extends JFrame {
    ...
    public Server5() {
        JFrame mainFrm = new JFrame("Hệ Thống Rạp Phim - Server 5");
        ...
        scroll.setBorder(
                BorderFactory.createTitledBorder(BorderFactory.createLineBorder(Color.blue, 2), "MÁY CHỦ 5 - RẠP PHIM",
    ...
    public static class sv5 implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // --- ĐIỂM KHÁC BIỆT CỦA SERVER 5: CHUYỀN VỀ LẠI SERVER 1 ---
                gs.sendUpdate("127.0.0.1", 2005, "Server5");

                ServerSocket server = new ServerSocket(2005); // Cổng của Server 5
                while (true) {
                    int localPort = server.getLocalPort();
                    display.append("Server 5 đang lắng nghe tại cổng " + localPort + ".\n");
                    Socket client = server.accept();
                    apps.handler(client, "Server5", 5, currentCircle, hash); // Pos = 5
                    apps.runServer();
    ...
    public static class getLamports implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                int max;
                while (true) {
                    String address = "224.0.255.1"; // IP Multicast của Server 5
                    ...
                    if (temp.startsWith("#")) {
                        try {
                            // Send Server (Gửi tới 4 IP còn lại)
                            String address1 = "224.0.0.0";
                            String address2 = "235.0.0.1";
                            String address3 = "235.255.0.1";
                            String address4 = "225.4.5.6";
